...
def buscar_global():
    ...
    sql = '''
        SELECT * FROM equipos 
        WHERE ns_torre LIKE %s OR ns_monitor LIKE %s OR id_inv_torre LIKE %s OR id_inv_monitor LIKE %s OR ubicacion LIKE %s
    ''' if IS_HEROKU else '''
        SELECT * FROM equipos 
        WHERE ns_torre LIKE ? OR ns_monitor LIKE ? OR id_inv_torre LIKE ? OR id_inv_monitor LIKE ? OR ubicacion LIKE ?
    '''
